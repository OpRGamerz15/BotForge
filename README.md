...
## Uploading via the GitHub website (no local git needed)

This project is flattened on purpose: `src/` has no subfolders, and the
whole repo only has one other folder (`.github/workflows/`, for the
auto-deploy file). Here's how to get it into GitHub using only the web
buttons, in order:

1. In your repo, click **Add file → Upload files** and drag in the root
   files: `package.json`, `package-lock.json`, `vite.config.js`,
   `index.html`, `README.md`, `.gitignore`, `.oxlintrc.json`. Commit.
2. Click **Add file → Create new file**. In the filename box type
   `src/App.jsx` (with the slash) — GitHub creates the `src` folder for
   you automatically. Paste in that file's contents. Commit.
3. Now that `src/` exists, open it, click **Add file → Upload files**,
   and drag in the *rest* of the files from your local `src/` folder all
   at once (they're all flat, no subfolders, so this is one plain
   multi-file upload). Commit.
4. Click **Add file → Create new file** again and type
   `.github/workflows/deploy.yml` as the filename — same trick, GitHub
   creates both nested folders for you. Paste in that file's contents.
   Commit.
5. Go to **Settings → Pages** and set Source to **GitHub Actions**.
6. Go to the **Actions** tab and confirm the "Deploy to GitHub Pages"
   workflow ran successfully (green check). Your site is now live at the
   URL shown in Settings → Pages.

## Getting started (local development instead)
